function learn(time, courses) {
   ...
   for(let i = 0; i < courses.length; i++) {
     for (let j=i+1; j<courses.length; j++) {
        ...
        if(distances[sum] === undefined && sum <= time) {
           distances[sum] = [i,j];
        }
     }
   }
   ...
   const minDistance = Math.max(...keys);
   const res = distances[minDistance].sort();
   return res;
  }
...
//   learn(4, [10, 14, 20]) // null -> no nos da tiempo a hacer dos cursos
  ...
  learn(10, [5, 5, 5]) // null -> no nos da tiempo a hacer dos cursos